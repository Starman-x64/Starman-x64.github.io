export const usedInRecipeTable = function(item, recipes, $i) {
    let itemDataToGet = [item.tag];
    let tableContents = "";
    recipes.forEach((recipeItem) => {
        if (!itemDataToGet.includes(recipeItem.item)) {
            itemDataToGet.push(recipeItem.item);
        }
        recipeItem.recipes.forEach((recipe) => {
            let ingredientsList = ""
            ...
            let tableRow = `<tr><th>${recipeItem.item}</th><td style="text-align:left;"><ul >${ingredientsList}</ul></td><td style="text-align:center;">${recipe.workStation}</td><td style="text-align:center;"><img src="../../_wiki_site_files/img/recipe/${recipe.workStation}/${recipeItem.item}.png" alt="${recipeItem.item} recipe" /></td></tr>`;
    ...
    tableContents = tableContents.replaceAll(new RegExp(`>${item.tag}<`, "g"), `><b>${dataToReplace[item.tag].name}</b><`);
